...
module.exports = (gulp, config, tasks) => {
  ...
  function compileJs(done) {
    gulp.src(config.js.src)
      .pipe($.if(config.js.sourceMap.enabled, $.sourcemaps.init()))
      .pipe($.if(config.js.babel, $.babel())) // all babel options handled in `.babelrc`
      .pipe($.if(config.js.concat, $.concat(config.js.destName)))
      .pipe($.if(config.js.uglify, $.uglify()))
      .pipe($.if(config.js.sourceMap.enabled, $.sourcemaps.write((config.js.sourceMapEmbed) ? null : './')))
